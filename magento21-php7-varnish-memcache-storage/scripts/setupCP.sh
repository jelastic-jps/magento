...
sed -i 's|.*extension=mysqlnd_ms.so|extension=mysqlnd_ms.so\nmysqlnd_ms.multi_master = 1|g' /etc/php.ini;
sed -i 's|.*mysqlnd_ms.enable = 1|mysqlnd_ms.enable = 1|g' /etc/php.ini;
sed -i 's|.*mysqlnd_ms.config_file.*|mysqlnd_ms.config_file=/var/lib/jelastic/mysqlnd_ms.json|g' /etc/php.ini;
